...
bare_extract:
	@docker run --name $(CONTAINER_NAME) ui-components pack --filename mojaloop-payment-manager-ui-components.tgz
	@docker cp $(CONTAINER_NAME):/usr/local/code/mojaloop-payment-manager-ui-components.tgz ./
	@docker rm $(CONTAINER_NAME)

upload_release:
	@set -e ;\
	VERSION=$$(docker run --rm ui-components -s printversion) ;\
	RELEASE="ui-components/mojaloop-payment-manager-ui-components-$$VERSION.tgz" ;\
	echo Releasing to https://pm4ml-release-artifacts.s3.amazonaws.com/$$RELEASE ;\
	aws s3 cp mojaloop-payment-manager-ui-components.tgz s3://pm4ml-release-artifacts/$$RELEASE --acl public-read
